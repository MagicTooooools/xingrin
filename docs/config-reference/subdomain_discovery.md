...
<!-- Generated At (UTC): 2026-01-21T03:28:30Z -->
...
**Description**: Collect subdomains using multiple search engines

**Base Command**:

```shell
python3 '/opt/orbit-tools/share/Sublist3r/sublist3r.py' -d {{.Domain}} -o {{quote .OutputFile}}
```
...
**Internal Parameters**:
- `resolvers-path-cli`: /opt/orbit/wordlists/resolvers.txt
...
recon:
  ...
  tools:
    ...
    subfinder:
      ...
      timeout-runtime: 3600  # Scan timeout in seconds
      threads-cli: 10  # Number of concurrent threads
    # Collect subdomains using multiple search engines
    sublist3r:
      enabled: true
      timeout-runtime: 3600  # Scan timeout in seconds
      threads-cli: 10  # Number of concurrent threads
