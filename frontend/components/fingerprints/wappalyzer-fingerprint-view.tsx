...
const WappalyzerFingerprintDialog = dynamic(
  () => import("./wappalyzer-fingerprint-dialog").then((mod) => mod.WappalyzerFingerprintDialog),
  { ssr: false }
)
...
export function WappalyzerFingerprintView() {
  ...
  const handleExport = async () => {
    try {
      ...
      downloadBlob(blob, `wappalyzer-fingerprints-${Date.now()}.json`)
