...
type RoleKind = "server" | "agent" | "worker"
...
const GROUP_BY_NODE_ID = VPS_GROUPS.reduce<Record<string, string>>(
  (acc, group) => {
...
function resolveLayout(
  ...
  const maxWorkersPerAgent = Math.max(
    1,
    ...workerGroups.map((group) => group.workerIds.length)
  )
  const gap = clamp(DEFAULT_NODE_GAP, MIN_HORIZONTAL_GAP, MAX_HORIZONTAL_GAP)
  const startX = ROLE_START.x

  const positions: Record<string, XYPosition> = {}
  const stepX = NODE_WIDTH + gap
  const groupStep = Math.max(GROUP_ROW_GAP, maxWorkersPerAgent * WORKER_STACK_GAP)
  const totalHeight =
    (agentIds.length - 1) * groupStep +
    (maxWorkersPerAgent - 1) * WORKER_STACK_GAP
  ...
  const groups = workerGroups.reduce<FlowLayout["groups"]>(
    (acc, group) => {
      const agentPosition = positions[group.agentId]
      if (!agentPosition) return acc

      const workerPositions = group.workerIds
        .map((workerId) => positions[workerId])
        .filter(Boolean)
      const minX = Math.min(
        agentPosition.x,
        ...workerPositions.map((position) => position.x)
      )
      const maxX = Math.max(
        agentPosition.x,
        ...workerPositions.map((position) => position.x)
      ...
      const minY = Math.min(
        agentPosition.y,
        ...workerPositions.map((position) => position.y)
      )
      const maxY = Math.max(
        agentPosition.y,
        ...workerPositions.map((position) => position.y)
      ...
      const height =
        maxY - minY + NODE_HEIGHT + GROUP_PADDING_TOP + GROUP_PADDING_BOTTOM
  ...
  const bottomY = Math.max(
    ...
    ...Object.values(groups).map(
      (group) => group.position.y + group.height
    )
  )
...
export function ArchitectureFlow() {
    ...
    () => {
      ...
      return [
        ...
        ...AGENT_IDS.map((id, index) =>
          buildRole(id, ROLE_TEMPLATES.agent, "agent", index + 1)
        ),
  ...
  const layout = useMemo(
    () => resolveLayout(size.width, LAYOUT_NODES),
    [size.width]
  )
