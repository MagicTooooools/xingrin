...
type DirectorySnapshotHandler struct {
	svc *service.DirectorySnapshotService
}

// NewDirectorySnapshotHandler creates a new directory snapshot handler
func NewDirectorySnapshotHandler(svc *service.DirectorySnapshotService) *DirectorySnapshotHandler {
...
func (h *DirectorySnapshotHandler) BulkUpsert(c *gin.Context) {
	...
	snapshotCount, assetCount, err := h.svc.SaveAndSync(scanID, req.TargetID, req.Directories)
	if err != nil {
...
func toDirectorySnapshotResponse(s *model.DirectorySnapshot) dto.DirectorySnapshotResponse {
