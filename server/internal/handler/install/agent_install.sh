...
if [ -z "$REGISTER_URL" ]; then
  echo "LUNAFOX_REGISTER_URL is required (no defaults)." >&2
  exit 1
fi
...
RESPONSE=$(curl "${curl_opts[@]}" \
  -X POST "$REGISTER_URL/api/agents/register" \
...
if [ "$SKIP_PULL" = "1" ] || [ "$SKIP_PULL" = "true" ]; then
  if image_exists "$AGENT_IMAGE:$AGENT_VERSION"; then
    ...
    echo "Local agent image not found and AGENT_SKIP_PULL is set." >&2
    exit 1
  fi
else
  if image_exists "$AGENT_IMAGE:$AGENT_VERSION"; then
...
if [ "$SKIP_PULL" = "1" ] || [ "$SKIP_PULL" = "true" ]; then
  if image_exists "$WORKER_IMAGE:$AGENT_VERSION"; then
    ...
    echo "Local worker image not found and AGENT_SKIP_PULL is set." >&2
    exit 1
  fi
else
  if image_exists "$WORKER_IMAGE:$AGENT_VERSION"; then
